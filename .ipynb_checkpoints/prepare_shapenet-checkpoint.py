...
def generate_dataset_with_chunk(mode='train', dataset_name='shapenet', cube_size=20, 
                               min_num=100, max_num=100000, save_path='./data/shapenet', chunk_size=500):
    ...
    for item_idx, item in enumerate(pcd_item):
        ...
        mask, points, meta_data = divide_cube(xyzs, attribute=normals, cube_size=cube_size, 
                                              min_num=min_num, max_num=max_num, 
                                              cur_sample_strategy=cur_sample_strategy)
        ...
        if (item_idx + 1) % chunk_size == 0 or item_idx == len(pcd_item) - 1:
            with open(os.path.join(save_path, f'{dataset_name}_{mode}_cube_size_{cube_size}_chunk_{chunk_idx}.pkl'), 'wb') as f:
...
def combine_chunks(mode='train', dataset_name='shapenet', cube_size=22, save_path='./data/shapenet'):
    combined_data = {}
    chunk_idx = 0

    while True:
        if chunk_idx < 3:
            chunk_file = os.path.join(save_path, f'{dataset_name}_{mode}_cube_size_{cube_size}_chunk_{chunk_idx}.pkl')
            if not os.path.exists(chunk_file):
                break

            with open(chunk_file, 'rb') as f:
                data_chunk = pkl.load(f)
                combined_data.update(data_chunk)
            chunk_idx += 1
        else:
            break

    with open(os.path.join(save_path, f'{dataset_name}_{mode}_cube_size_{cube_size}_combined.pkl'), 'wb') as f:
        pkl.dump(combined_data, f, protocol=2)

def parse_dataset_args():
    parser = argparse.ArgumentParser(description='Shapenet Dataset Arguments')

    # data root
    parser.add_argument('--data_root', default='./', type=str, help='dir of shapenet core dataset')
